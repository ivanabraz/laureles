                    ...
                    {dates.length > 0 ? (
                        dates.map((date, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`grid grid-cols-12 pb-2 border-b border-neutral-500 text-base xs:text-xs sm:text-sm md:text-base ${isPastEvent ? 'text-neutral-400' : ''}`}
                                >
                                    {/* Día */}
                                    <div className="col-span-1 text-center">{date.Day}</div>
                                    {/* Mes */}
                                    <div className="col-span-1 text-center">{date.Month.slice(0, 3).toUpperCase()}</div>
                                    {/* Año */}
                                    <div className="col-span-1 text-center">{date.Year}</div>
                                    {/* Nombre del lugar */}
                                    <div className="col-span-4 text-left">{date.Venue}</div>
                                    {/* Ciudad, País */}
                                    <div className="col-span-4 text-left">{date.Place}</div>
                                    {/* Tickets */}
